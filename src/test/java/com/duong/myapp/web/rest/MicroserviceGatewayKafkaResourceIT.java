...
import com.duong.myapp.config.KafkaProperties;
...
class MicroserviceGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        MicroserviceGatewayKafkaResource kafkaResource = new MicroserviceGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/microservice-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/microservice-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
